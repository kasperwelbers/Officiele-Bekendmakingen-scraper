...
import datetime, sys, urllib2, re
...
class HandelingenPerSprekerScraper(OfficieleBekendmakingenScraper):
    ...
    def scrape_unit(self, url):
        ...
        if len(metadict) == 0:
            print("NO METADATA FOR %s. SKIPPING URL" % url)
            return
        ...
        try: omschrijving = xml.cssselect('itemkop')[0].text_content()
            ...
            try: omschrijving = xml.cssselect('onderwerp')[0].text_content()
                ...
                try: omschrijving = metadict['DC.title']
                except: omschrijving = metadict['dcterms.title']

        #metabody = '\n'.join(["%s:\t%s;" % (meta, metadict[meta]) for meta in metadict])
        parentbody = omschrijving
        if parentbody == '': parentbody = 'body_missing'

        parent = dict(headline=document_id, byline=itemnaam, text=parentbody, date=date, section=section, url=url, pagenr=0, medium=self.medium_name)
        yield parent

        for nr, sprekerdict in enumerate(self.sprekerDictReader(xml)):
            spreker = self.printSpreker(sprekerdict)
            #print('\t{spreker}'.format(spreker=spreker))
            if sprekerdict['tekst'] == '': sprekerdict['tekst'] = 'body_missing'
            headline_spreker = "%s - #%s | %s" % (document_id, nr+1, spreker)
            yield dict(headline=headline_spreker, author=spreker, byline=itemnaam, text=sprekerdict['tekst'], date=date, section=section, url="%s#%s" % (url, nr+1), pagenr=nr+1, medium=self.medium_name)

            for motie in sprekerdict['moties']:
                yield dict(headline="%s | MOTIE" % headline_spreker, author=spreker, byline=itemnaam, text=motie, date=date, section=section, url="%s#%s" % (url, nr+1), pagenr=nr+1, medium=self.medium_name)
        #return []
...
if __name__ == '__main__':
    s = HandelingenPerSprekerScraper()
    s.scrape()
